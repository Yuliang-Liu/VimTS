...
class MaxViT(Backbone):
    ...
    def __init__(
        ...
        for index, (depth, channel) in enumerate(zip(depths, channels)):
            ...
            stage = f'stage{index+2}'
            if stage in self.out_features:
                self._out_feature_channels[stage] = channel
                self._out_feature_strides[stage] = 4 * 2 ** index
...
def max_vit_tiny_224(cfg) -> MaxViT:
    """ MaxViT tiny for a resolution of 224 X 224"""
    out_features = cfg.MODEL.MAXVIT.OUT_FEATURES
    return MaxViT(
        depths=(2, 2, 5, 2),
...
def build_maxvit_backbone(cfg, input_shape):
    """
    ...
    return max_vit_tiny_224(cfg)


@BACKBONE_REGISTRY.register()
def build_maxvit_fpn_backbone(cfg, input_shape: ShapeSpec):
    """
    Args:
    ...
    """
    bottom_up = build_maxvit_backbone(cfg, input_shape)
